...
function updateMapInhaltText(features, date, name) {
    ...
    if (mapInhaltTextDiv) {
        ...
        if (filteredFeatures.length > 0) {
            // Funktion, um den Link-Text mit optionalem "(Wohnsitz)" zu generieren
            const createLinkText = (feature) => {
                const title = feature.properties.title || feature.properties.id;
                // HIER wird checkDateInRange aufgerufen!
                const istWohnsitz = checkDateInRange(date, feature.properties.id);
                return `${title}${istWohnsitz ? ' (Wohnsitz)' : ''}`; // Fügt "(Wohnsitz)" hinzu, wenn true
            };
            ...
            if (wohnsitz) {
                // Hinweis: Hier wird bereits der Wohnsitz angezeigt, aber nur wenn *keine* anderen Orte bekannt sind.
...
function clearGeoJsonLayers(date) {
    // Standardwert für 'date' manuell setzen, falls kein Wert übergeben wurde
    var effectiveDate = (typeof date === 'undefined' || date === null) ? '' : date;

    // Hinweis: Die forEach-Methode mit Arrow Function ist ebenfalls ES6.
    // Für maximale Kompatibilität könnte man auch das anpassen:
    ...
    // Den effektiven (ggf. Standard-) Wert verwenden
    updateMapInhaltText([], effectiveDate);
}
