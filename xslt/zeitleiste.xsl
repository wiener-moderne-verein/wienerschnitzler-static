    ...
    <xsl:template match="/">
        <html class="h-100">
            ...
            <head>
                ...
                <xsl:variable name="doc_title" select="local:translate('zeitleiste.title')"/>
                <xsl:variable name="doc_description" select="local:translate('zeitleiste.meta_description')"/>
            ...
            <body class="h-100">
                ...
                <main class="w-100">
                    <div class="container-fluid my-4" id="main-content">
                        <h1><xsl:value-of select="local:translate('zeitleiste.heading')"/></h1>
                        <div class="form-group">
                            <label for="yearSelect"><xsl:value-of select="local:translate('zeitleiste.year_select')"/></label>
                            <select id="yearSelect"></select>
                            <div style="width: 20px"></div>
                            <input type="checkbox" id="toggleAdditional" />
                            <label for="toggleAdditional" class="ms-2"><xsl:value-of select="local:translate('zeitleiste.toggle_additional')"/></label>
                        </div>
                       <div class="center"> 
